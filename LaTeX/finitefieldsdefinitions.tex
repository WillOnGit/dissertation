...
$$ax \equiv 1 \mod n,$$
where \Cref{congruencesolutions} clearly implies that $a$ and $n$ must be coprime for an inverse to exist. Therefore all elements $\{1,\ldots,n-1\}$ must be coprime with $n$ for $\zn$ to be a field. Clearly, this happens if and only if $n$ is prime.

% This tells us not only why $n$ must be prime for $\zn$ to be a field, but also what happens when it is composite: namely, all elements which are coprime to $n$ will still have an inverse.
% Thus, $\zn$ will ``sort of work'' as a field.
% This idea will be revisited later.
